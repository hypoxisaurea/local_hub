...
이 프로젝트는 Python 코드가 `backend/`, `AI/`에 나뉘어 있지만, Python 가상환경은 프로젝트 루트의 `.venv` 하나만 사용합니다. 하위 폴더마다 `.venv`를 따로 만들지 않습니다.
...
단, `AI/.venv`, `backend/.venv` 같은 하위 가상환경은 만들지 않습니다.
